...
export default class Simulator {
  constructor(domElement) {
    this.pathPlannerWorker = new Worker(
      URL.createObjectURL(
        new Blob([`(${dash_initPathPlannerWorker.toString()})()`], {
    ...
    this.manualModeButton.addEventListener(
      "click",
      this.enableManualMode.bind(this)
    );
    this.autonomousModeButton = document.getElementById("mode-autonomous");
    this.autonomousModeButton.addEventListener(
      "click",
      this.enableAutonomousMode.bind(this)
    );

    document
      .getElementById("editor-enable")
      .addEventListener("click", this.enableEditor.bind(this));
    document
      .getElementById("editor-finalize")
      .addEventListener("click", this.finalizeEditor.bind(this));
    document
      .getElementById("simulator-load")
      .addEventListener("click", this.loadScenario.bind(this));

    this.scenarioPlayButton = document.getElementById("scenario-play");
    this.scenarioPlayButton.addEventListener(
      "click",
      this.playScenario.bind(this)
    );
    this.scenarioPauseButton = document.getElementById("scenario-pause");
    this.scenarioPauseButton.addEventListener(
      "click",
      this.pauseScenario.bind(this)
    );
    this.scenarioRestartButton = document.getElementById("scenario-restart");
    this.scenarioRestartButton.addEventListener(
      "click",
      this.restartScenario.bind(this)
    );

    this.welcomeModal = document.getElementById("welcome-modal");
    document
      .getElementById("show-welcome-modal")
    ...
    document
      .getElementById("welcome-modal-background")
      .addEventListener("click", this.hideWelcomeModal.bind(this));
    document
      .getElementById("welcome-modal-close")
      .addEventListener("click", this.hideWelcomeModal.bind(this));

    document
      .getElementById("welcome-modal-examples")
      .addEventListener("click", (e) => {
        this.welcomeModal.classList.remove("is-active");
        this.loadScenario();
        this.editor.scenarioManager.switchTab(
          this.editor.scenarioManager.examplesTab
        );
      });

    document
      .getElementById("welcome-modal-create")
      .addEventListener("click", (e) => {
        this.welcomeModal.classList.remove("is-active");
        this.enableEditor();
      });

    this.simModeBoxes = Array.prototype.slice.call(
      document.getElementsByClassName("sim-mode-box"),
      0
    );
    this.editModeBoxes = Array.prototype.slice.call(
      document.getElementsByClassName("edit-mode-box"),
      0
    );
  ...
  toss() {
    ...
    const pos = rotVec
      .clone()
      .multiplyScalar(50)
      .add(new THREE.Vector2(rotVec.y, rotVec.x))
      .add(pose.pos);
  ...
  _setUpCameras(domElement) {
    ...
    this.topDownControls = new TopDownCameraControls(
      domElement,
      this.topDownCamera
    );
    ...
    this.editorCameraControls = new TopDownCameraControls(
      domElement,
      this.editorCamera
    );
  ...
  _resetChaseCamera() {
    ...
    const dirVector = THREE.Vector2.fromAngle(this.car.rotation).multiplyScalar(
      -20
    );
  ...
  finalizeEditor(replaceCamera = true) {
    ...
    if (this.editor.lanePath.anchors.length > 1) {
      ...
      const perpindicular =
        rot + (Math.PI / 2) * (Math.sign(this.editor.lanePreference) || 0);
      const latitude = this.pathPlannerConfigEditor.config.roadWidth / 4;

      this.car.setPose(
        pos.x + Math.cos(perpindicular) * latitude,
        pos.y + Math.sin(perpindicular) * latitude,
        rot
      );
  ...
  restartScenario() {
    ...
    if (this.plannedPathGroup) this.scene.remove(this.plannedPathGroup);
  ...
  changeCamera(mode) {
    ...
    switch (mode) {
      case "free":
        ...
        if (this.camera == this.freeCamera) this._resetFreeCamera();
        else this.camera = this.freeCamera;
      ...
      case "chase":
        ...
        if (this.camera == this.chaseCamera) this._resetChaseCamera();
        else this.camera = this.chaseCamera;
      ...
      case "topDown":
        ...
        if (this.camera == this.topDownCamera) this._resetTopDownCamera();
        else this.camera = this.topDownCamera;
  ...
  startPlanner(pose, station) {
    ...
    if (
      ...
      predictedPose = this.autonomousCarController.predictPoseAfterTime(
        pose,
        latency
      );
      let [predictedStation] = this.editor.lanePath.stationLatitudeFromPosition(
        predictedPose.pos,
        this.aroundAnchorIndex
      );
      startTime += latency;
    }
    ...
    this.lastPlanParams = {
      ...
      dynamicObstacles: this.dynamicObstacles.filter(
        (o) => o.positionAtTime(startTime).x >= 0
      ),
      reset: reset,
    };
  ...
  receivePlannedPath(event) {
    ...
    this.averagePlanTime.addSample(
      (performance.now() - this.lastPlanTime) / 1000
    );
    this.plannerReady = true;
    ...
    const lattice = new RoadLattice(
      this.editor.lanePath,
      latticeStartStation,
      config
    );
    ...
    if (false && dynamicObstacleGrid) {
      const dynamicGridTex = new THREE.DataTexture(
        dynamicObstacleGrid.data,
        dynamicObstacleGrid.width,
        dynamicObstacleGrid.height,
        THREE.RGBAFormat,
        THREE.FloatType
      );
      ...
      const [gridStart] = this.editor.lanePath.sampleStations(
        vehicleStation,
        1,
        0
      );
      if (gridStart) {
        const dynamicGridGeom = new THREE.PlaneGeometry(
          dynamicObstacleGrid.width * config.slGridCellSize,
          dynamicObstacleGrid.height * config.slGridCellSize
        );
        const dynamicGridMat = new THREE.MeshBasicMaterial({
          map: dynamicGridTex,
          depthTest: false,
          transparent: true,
          opacity: 0.5,
        ...
        const offset = THREE.Vector2.fromAngle(gridStart.rot).multiplyScalar(
          (dynamicObstacleGrid.width * config.slGridCellSize) / 2 -
            config.spatialHorizon / config.lattice.numStations
        );
        dynamicGridObj.position.set(
          gridStart.pos.x + offset.x,
          0,
          gridStart.pos.y + offset.y
        );
    ...
    if (fromVehicleParams.type == "cubic") {
      ...
      if (pathBuilder.optimize()) {
        fromVehicleSegment = pathBuilder.buildPath(
          Math.ceil(pathBuilder.params.sG / 0.25)
        );

        const prevVelocitySq = this.car.velocity * this.car.velocity;
        const accel =
          (end.velocity * end.velocity - prevVelocitySq) /
          2 /
          pathBuilder.params.sG;
        const ds = pathBuilder.params.sG / (fromVehicleSegment.length - 1);
        let s = 0;

        for (let p = 0; p < fromVehicleSegment.length; p++) {
          fromVehicleSegment[p].velocity = Math.sqrt(
            2 * accel * s + prevVelocitySq
          );
    ...
    pathGeometry.setFromPoints(
      path.map((p) => new THREE.Vector3(p.pos.x, 0, p.pos.y))
    );
    const pathLine = new MeshLine();
    pathLine.setGeometry(pathGeometry);

    const color =
      fromVehicleParams.type == "cubic"
        ? new THREE.Color(0xff8800)
        : new THREE.Color(0xffff40);
  ...
  step(timestamp) {
    ...
    if (!this.editor.enabled && !this.paused) {
      ...
      const manualControls = this.manualCarController.control(
        this.car.pose,
        this.car.wheelAngle,
        this.car.velocity,
        dt
      ...
      if (
        manualControls.steer != 0 ||
        manualControls.brake != 0 ||
        manualControls.gas != 0
      )
        this.enableManualMode();

      let autonomousControls = { steer: 0, brake: 0, gas: 0 };
      if (this.autonomousCarController)
        autonomousControls = this.autonomousCarController.control(
          this.car.pose,
          this.car.wheelAngle,
          this.car.velocity,
          dt,
      ...
      const positionOffset = {
        x: carPosition.x - prevCarPosition.x,
        y: 0,
        z: carPosition.y - prevCarPosition.y,
      };
      ...
      if (this.editor.lanePath.anchors.length > 1) {
        const [
          ...
          l,
          aroundAnchorIndex,
        ] = this.editor.lanePath.stationLatitudeFromPosition(
          carRearAxle,
          this.aroundAnchorIndex
        );
